...
uint32_t bignum_to_uint32(struct bn* n)
{
  require(n, "n is null");
...
void bignum_invert(struct bn* a, struct bn* m, struct bn* c)
{
  /*
    Return a multiplicative inverse such that ac % m == 1
  */
    require(a, "a is null");
    require(m, "b is null");
    require(c, "c is null");
    ...
    struct bn tmp;
    bignum_init(&tmp);
    if (bignum_cmp(&M, &cnst1) == EQUAL){
        return 0;
    }
    while (bignum_cmp(&A, &cnst1) == LARGER) {
        // q is quotient

        bignum_init(&q);
        bignum_divmod(&A, &M, &q, &tmp);
        bignum_init(&t);
        bignum_assign(&t,&M);
...
void bignum_mod_neg(struct bn* a, struct bn* b, struct bn* c)
{
  /*
    Take divmod and throw away div part
  ...
  struct bn tmp2;
  struct bn tmpthrow;
  bignum_divmod(a,b,&tmpthrow,&tmp);
  bignum_add(&tmp,b,&tmp2);
  bignum_divmod(&tmp2,b,&tmpthrow,c);
  */
  require(a, "a is null");
  require(b, "b is null");
  require(c, "c is null");
